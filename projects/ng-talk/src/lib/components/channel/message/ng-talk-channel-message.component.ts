...
@Component({
  ...
  template: `
    @if (chat.settings().showAvatars && showAuthor) {
      <div class="avatar">
        <img
          [src]="message.from().avatar ?? chat.settings().defaultAvatar"
          (click)="chat.userClicked.emit(message.from())"
        />
      </div>
    }
    <div class="speech-balloon">
      <!-- Actions menu -->
      @if (
        message.type != MessageType.Writing && chat.settings().allowReplies
      ) {
        <span
      ...
      <!-- Replied message -->
      @if (message.replyTo) {
        <ng-talk-channel-message-ref
          role="button"
          [message]="message.replyTo"
          (click)="chat.goToMessage(message.replyTo)"
        />
      }
      ...
      @if (chat.settings().showNames && showAuthor) {
        <div
          class="author"
          [style.color]="message.from().color"
          (click)="chat.userClicked.emit(message.from())"
        >
          {{ message.from().name }}
        </div>
...
export class NgTalkChannelMessageComponent implements OnChanges, OnDestroy {
  ...
  @HostBinding('class') protected _className: string;
  ...
  private _touchEventSubscription = fromEvent(
    ...
    'touchstart',
    normalizePassiveListenerOptions({ passive: true }) as any
  ...
  public ngOnChanges() {
    ...
    this.showAuthor =
      !prevMessage ||
      prevMessage.from().id != this.message.from().id ||
      !isSameDay(prevMessage.date, this.message.date);

    this._className =
      this.chat.settings().messageClass +
  ...
  private _onTouchStart() {
    this._touchEventSubscription.add(
      fromEvent(this._host.nativeElement, 'touchend').subscribe(() =>
        clearTimeout(this._showTimeout)
      )
    );
    this._touchEventSubscription.add(
      fromEvent(this._host.nativeElement, 'touchmove').subscribe(() =>
        clearTimeout(this._showTimeout)
      )
    );
