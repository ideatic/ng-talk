import {ChangeDetectionStrategy, Component, ElementRef, HostBinding, inject, Input, OnChanges, OnDestroy, viewChild, input} from "@angular/core";
...
export class NgTalkChannelMessageComponent implements OnChanges, OnDestroy {
  ...
  public readonly prevMessage = input<ChatMessage>(undefined);
  ...
  public ngOnChanges() {
    ...
    this.showAuthor = !prevMessage || prevMessage.from().id != this.message.from().id || !isSameDay(prevMessage.date, this.message.date);
