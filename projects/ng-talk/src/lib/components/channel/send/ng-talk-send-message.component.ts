  ...
  template: `
    @if (chat.channel?.blocked) {
      ...
      <form (ngSubmit)="sendTextMessage()">
        <!-- Emoji -->
        <span role="button" class="emoji-btn" [style.color]="showEmojiSelector ? '#008069' : null"
              (click)="showGifSelector = false; showEmojiSelector = !showEmojiSelector">
        ...
        @if (chat.settings.giphyApiKey) {
          <span
            role="button" class="emoji-btn" [style.color]="showGifSelector ? '#008069' : null"
            (click)="showEmojiSelector = false; showGifSelector = !showGifSelector">
          <svg viewBox="0 0 24 24" width="24" height="24">
            <path fill="currentColor"
        ...
        <!-- Text -->
        <input #textInput class="input" name="newMessage" maxlength="1000" autocomplete="off" [placeholder]="chat.settings.writePlaceholder"
               [disabled]="!chat.channel || chat.channel.disabled" [(ngModel)]="newMessage"
               (focus)="onInputFocus()"/>
        <span role="button" class="send-btn" [class.disabled]="!newMessage || !chat.channel || chat.channel.disabled" [title]="chat.settings.sendBtnTitle"
