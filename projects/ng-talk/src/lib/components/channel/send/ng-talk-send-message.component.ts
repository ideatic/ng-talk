    ...
    @if (chat.channel()?.blocked) {
      ...
      @if (mediaSelector == 'emoji') {
        <ng-talk-send-emoji
          ...
          (emojiSelected)="newMessage = newMessage + $event"
        />
      } @else if (mediaSelector == 'gif') {
        <ng-talk-send-gif
          class="grow-animation"
          animate.leave="leaving"
          (gifSelected)="sendPhoto($event)"
        />
      }
      ...
      <form (ngSubmit)="sendTextMessage()">
        ...
        <span
          role="button"
          class="emoji-btn"
          [style.color]="mediaSelector == 'emoji' ? '#008069' : null"
        ...
        @if (chat.settings().giphyApiKey || chat.settings().tenorApiKey) {
          <span
            role="button"
            class="emoji-btn"
            [style.color]="mediaSelector == 'gif' ? '#008069' : null"
        ...
        <span
          ...
          class="send-btn"
          [class.disabled]="
            !newMessage || !chat.channel() || chat.channel().disabled
          "
...
export class NgTalkSendMessageComponent {
  ...
  private readonly _textInput = viewChild('textInput', {
  ...
  constructor() {
    ...
    if (channelList) {
      // Detectar cambio de canal si estamos en un listado
      const subscription = channelList.channelChanged.subscribe(c =>
        this._onChannelChanged(c)
      );
  ...
  private _sendMessage(message: Partial<ChatMessage>) {
    if (this.chat.channel() && !this.chat.channel().disabled) {
      ...
      this.chat
        .adapter()
        .sendMessage(this.chat.channel(), fullMessage)
