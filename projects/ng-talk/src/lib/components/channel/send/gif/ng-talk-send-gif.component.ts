...
import { DebounceDirective } from '@ng-shared/ui/debounce.directive';
import { FnPipe } from '../../../../pipes/fn.pipe';
import { NgTalkChannelComponent } from '../../ng-talk-channel.component';

@Component({
  selector: 'ng-talk-send-gif',
  imports: [FormsModule, DebounceDirective, FnPipe],
  template: `
    <div style="display: flex; align-items: center; margin-bottom: 10px">
      <input
        type="search"
        style="flex-grow: 1"
        [placeholder]="chat.settings().search"
        [debounce]="500"
        [(ngModel)]="searchQuery"
        (bounced)="debouncedSearchQuery.set($event ?? '')"
      />
    ...
    <div class="gif-list">
      @for (gif of gifs.value(); track gif) {
...
export class NgTalkSendGifComponent {
  // Deps
  ...
  protected readonly debouncedSearchQuery = signal('');

  protected readonly gifs = resource({
    params: () => {
      ...
      if (settings.giphyApiKey) {
        if (query) {
          return `https://api.giphy.com/v1/gifs/search?rating=g&q=${encodeURIComponent(query)}&api_key=${encodeURIComponent(settings.giphyApiKey)}`;
        } else {
          return `https://api.giphy.com/v1/gifs/trending?rating=g&api_key=${settings.giphyApiKey}`;
        }
      }

      if (settings.tenorApiKey) {
        const apiKey = encodeURIComponent(settings.tenorApiKey);
        const baseParams = `key=${apiKey}&client_key=ngTalk&limit=8`;
        if (query) {
          return `https://tenor.googleapis.com/v2/search?q=${encodeURIComponent(query)}&${baseParams}`;
        } else {
          return `https://tenor.googleapis.com/v2/featured?${baseParams}`;
        }
      }
  ...
  protected onGifSelected(gif: any) {
    ...
    if (this.chat.settings().giphyApiKey && gif.analytics?.onclick?.url) {
      fetch(gif.analytics.onclick.url);
    } else if (this.chat.settings().tenorApiKey) {
      fetch(
        `https://tenor.googleapis.com/v2/registershare?id=16989471141791455574&key=${encodeURIComponent(this.chat.settings().tenorApiKey)}&client_key=ngTalk&q=${encodeURIComponent(this.searchQuery)}`
      );
    }
  }
}
