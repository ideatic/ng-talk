...
@Component({
  ...
  template: `
    ...
    @if (!bubbleChannelSvc.hasInstance(chat.channel())) {
      <a class="tool" (click)="openBubbleChat(chat.channel())"
        ><i class="fas fa-external-link-square-alt"></i
      ></a>
    }
    <a class="tool" (click)="toggleBlock()"
      ><i
        class="fas"
        [class]="chat.channel().blocked ? 'fa-unlock' : 'fa-ban'"
      ></i
    ></a>
...
export class NgTalkChannelHeaderComponent {
  ...
  protected deleteChannel() {
    this.chat
      .adapter()
      .deleteChannel(this.chat.channel())
