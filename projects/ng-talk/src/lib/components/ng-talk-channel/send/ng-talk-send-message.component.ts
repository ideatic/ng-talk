  ...
  template: `
    ...
    <ng-template #sendMessageForm>
      ...
      <form (ngSubmit)="sendTextMessage()">
        <!-- Emoji -->
        <span role="button" (click)="showGifSelector = false; showEmojiSelector = !showEmojiSelector" class="emoji-btn" [style.color]="showEmojiSelector ? '#008069' : null">
        ...
        <!-- Gif -->
        <span *ngIf="chat.settings.giphyApiKey"
              role="button" (click)="showEmojiSelector = false; showGifSelector = !showGifSelector" class="emoji-btn" [style.color]="showGifSelector ? '#008069' : null">
          <svg viewBox="0 0 24 24" width="24" height="24">
            <path fill="currentColor"
